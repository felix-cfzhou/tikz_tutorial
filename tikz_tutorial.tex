...
\newpage
\section{Graphs}
This is definitely my favourite part of TikZ.
...
By default, nodes have non-zero sizes as TikZ has default non-zero inner sep between text and node boundaries.
We set this to 0 and instead use the minimum size command which allows the nodes to shrink to a minimum size when there is no text and expand as needed.

\begin{figure}[H]
    \centering
    \tikzstyle{place}=[circle,draw=blue!50,fill=blue!20,thick,inner sep=0,minimum size=0.5cm]
    \tikzstyle{transition}=[rectangle,draw=black!50,fill=black!20,thick,inner sep=0,minimum size=0.5cm]
    \begin{tikzpicture}
        \node at ( 0,2) [place] {};
        \node at ( 0,1) [place] {};
        \node at ( 0,0) [place] {};
        ...
        \node at (-1,1) [transition] {};
    \end{tikzpicture}
    \caption{Basic Sized Nodes}
    \label{fig:basicsizednodes}
\end{figure}
...
First we name the nodes, then describe their position relative to one another, the value node distance is important in our case.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=2cm]
        \node[place] (waiting) {};
        \node[place] (critical) [below left of=waiting] {};
        \node[place] (semaphore) [below left of=critical] {};
        \node[transition] (leave critical) [below right of=critical] {};
        \node[transition] (enter critical) [left of=critical] {};
    \end{tikzpicture}
    \caption{Relative Named Nodes}
    \label{fig:relativenamednodes}
\end{figure}
...
\subsection{Labels}
We can always explicitly define blank nodes as labels but the label option is much better suited to our needs.

\begin{figure}[H]
    \centering
    \tikzstyle{place}=[circle,draw=blue!50,fill=blue!20,thick,inner sep=0.1cm,minimum size=0.5cm]
    \tikzstyle{transition}=[rectangle,draw=black!50,fill=black!20,thick,inner sep=0.1cm,minimum size=0.5cm]
    \begin{tikzpicture}[node distance=2cm]
        \node[place] (waiting) [label=above:$3\cdot 23$] {$69$};
        \node[place] (critical) [below left of=waiting] {};
        \node[place] (semaphore) [below left of=critical,label=below left:$2^{10}$] {$1024$};
        \node[transition] (leave critical) [below right of=critical] {};
        \node[transition] (enter critical) [left of=critical,label=left:prime!] {$13$};
    \end{tikzpicture}
    \caption{Basic Labeled Nodes}
    \label{fig:basiclabelednodes}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \node[transition] (leave critical) [below right of=critical] {};
        \node[transition] (enter critical) [left of=critical,label=left:prime!] {$13$};
        \draw[->] (enter critical) -- (critical.west);
        \draw[->] (waiting) .. controls +(-5, 1) and +(-5, -1) .. (semaphore.west);
        \draw[<-] (waiting) to [out=270,in=0] (critical);
        \draw[<->] (critical) to [bend right=90] (leave critical);
    \end{tikzpicture}
    \caption{Basic Connected Nodes}
    \label{fig:basicconnectednodes}
\end{figure}
...
Here we showcase three of the many ways to connect nodes: through explicit paths, through indicating exit points, and through the bend command.

\subsection{Snake Edges}
\begin{figure}[H]
    \centering
    \tikz \draw[->,decorate,decoration={snake}] (0, 0) -- (10, 0);
    \caption{Basic Snake Edge}
    \label{fig:basicsnakeedge}
\end{figure}
...
We can control the behavior of the edge like below

\begin{figure}[H]
    \centering
    \tikz \draw[->,decorate,decoration={snake,amplitude=.4mm,segment length=2mm}] (0, 0) -- (10, 0);
    \caption{Adjusted Snake Edge}
    \label{fig:adjustedsnakeedge}
\end{figure}
...
One powerful mechanism of TikZ are transformations
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \filldraw[fill=Violet,rotate=45] (0.5, 0.5) rectangle (1, 1);
    \end{tikzpicture}
    \caption{Basic Transformations}
    \label{fig:basictransformations}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \foreach \x in {1, 1.25, 1.5, 1.75, 2}
            \draw (\x, 0) circle (\x);
    \end{tikzpicture}
    \caption{Basic Repetitions}
    \label{fig:basicrepetitions}
\end{figure}
...
\subsection{Layers}
What if we wish to populate the background after drawing some nodes of shapes?

\begin{figure}[H]
    \centering
    \pgfdeclarelayer{background}
    \pgfsetlayers{background,main}
    \tikzstyle{basic}=[draw,fill=Violet!20]
    \begin{tikzpicture}
        \node[basic] (N) at (0, 1) {N};
        \node[basic] (S) at (0, -1) {S};
        \node[basic] (E) at (1, 0) {E};
        \node[basic] (W) at (-1, 0) {W};

        \begin{pgfonlayer}{background}
            \filldraw[fill=black!30,draw=red]
                (S.south -| W.west) rectangle
                (N.north -| E.east);
        \end{pgfonlayer}
    \end{tikzpicture}
    \caption{Basic Background Layer}
    \label{fig:basicbackgroundlayer}
\end{figure}
